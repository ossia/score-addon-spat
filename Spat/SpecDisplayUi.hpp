...
struct SpecDisplay::ui
{
  ...
  halp_meta(name, "Main") halp_meta(layout, halp::layouts::grid)
      halp_meta(columns, 3) halp_meta(width, 600) halp_meta(height, 300)
  ...
  struct
  {
    halp_meta(name, "Spectral") halp_meta(layout, halp::layouts::vbox)
        halp_meta(background, halp::colors::mid) halp_meta(width, 300)
            halp_meta(height, 300)

                halp::custom_actions_item<custom_spec_display> widget{
                    .x = 0,
                    .y = 0};
  } spectral;
